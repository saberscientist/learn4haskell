...
closestToZero x y = if (0 - x) < (0 - y) then x else y
...
sumLast :: Int -> Int
sumLast n = tensDigit + onesDigit
  where
    tensDigit = div (mod n 100) 10
    onesDigit = mod n 10
...
firstDigit :: Int -> Int
firstDigit n = if x < 10 then x else firstDigit x
  where x = if n < 10 then n else (div n 10)
